...
### Requirement: GET /api/albumes/{albumId}/ganadores returns latest winners
The API SHALL provide `GET /api/albumes/{albumId}/ganadores` that returns an `ApiResponseDTO<List<GanadorAlbumDTO>>` with `success=true` and the list of active winners filtered by album.

#### Scenario: Fetch winners for album
- **WHEN** a client requests `/api/albumes/42/ganadores` and album 42 exists with 3 active winners
...
- **WHEN** a client requests `/api/albumes/9999/ganadores` but album 9999 does not exist
